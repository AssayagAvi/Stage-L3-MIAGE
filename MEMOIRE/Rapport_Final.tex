...
\end{itemize}
\section{Gurobi}
\paragraph{ Voici les deux méthodes (via gurobi.sh ou alors via le module gurobipy) que l'on peut utiliser pour résoudre un programme MIP nommé "mip.py" (voir exmeple [5.3]) :}
\begin{center}
\begin{figure}[h]
\centering
\includegraphics[scale=0.45]{gurobish.png}
\caption{Résolution via shell gurobi (gurobi.sh)}
\end{figure}
\begin{figure}[h]
\centering
\includegraphics[scale=0.45]{gurobipython.png}
